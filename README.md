...
1. **作成したフォルダーをCursorで開く**

   - 作成されたスライドフォルダーをCursorで開く
2. **slide.mdファイルを開く**

   - フォルダー内の`slide.md`ファイルをクリック
3. **AI機能でスライドを生成**
   ...
   **実際の操作：**
   ...
   - ファイル全体を選択（`Cmd+A` / `Ctrl+A`）
   - `Cmd+K`（Windows: `Ctrl+K`）でAI編集を開始
   - 以下のようにプロンプトを入力：

   ```
   マーケティング戦略について5枚のスライドをMarp形式で作成してください。
   既存のテンプレート内容は削除して、新しい内容で置き換えてください。
   ```
4. **AI機能の活用例**
